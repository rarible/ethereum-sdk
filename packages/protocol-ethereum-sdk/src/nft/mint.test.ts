...
const { web3, providers } = createTestProviders(provider, wallet)
...
//todo fix for other providers
describe.each([providers[0]])("mint test", ethereum => {
	...
	test("mint ERC-721 v1", async () => {
		...
		cnt = cnt + 1
		expect(balanceOfMinter).toBe(`${cnt}`)
	}, 20000)
