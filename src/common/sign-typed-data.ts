...
export async function signTypedData(web3: Web3, signer: string, data: any) {
	...
	try {
		...
		try {
			return await tryToSign(SignTypedDataTypes.SIGN_TYPED_DATA_V3, JSON.stringify(data))
		} catch (error) {
			try {
				return await tryToSign(SignTypedDataTypes.SIGN_TYPED_DATA, JSON.stringify(data))
			} catch (error) {
